...
import {
  useGetMyOrders,
  useGetOrders,
} from "@/hooks/react-query/useOrderMutation";
import { useAuth } from "@/hooks/useAuth";

const UserSoleEssentialOrders = () => {
  const { user } = useAuth();
  ...
  const filteredOrders = React.useMemo(() => {
    if (!data || !user) return [];
    let orders = data.filter((order: any) => order.userId === user.id);
  ...
  const renderOrderItem = ({ item }: { item: any }) => (
    <TouchableOpacity
      onPress={() =>
        router.push({
          pathname: "/user/essential-order-details",
...
export default UserSoleEssentialOrders;
